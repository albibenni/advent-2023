...
console.log(
  `Part 1 answer: ${part1_answer} in ${(
    performance.now() - part1_start
  ).toFixed(2)}ms`,
);
// Answer: 14526
...
console.log(
  `Part 2 answer: ${part2_answer} in ${(
    performance.now() - part2_start
  ).toFixed(2)}ms`,
);

console.log(
  `Solution completed in ${(performance.now() - solution_start).toFixed(2)}ms`,
);
...
function shuffle_arr(input, no_of_shuffles = 1) {
  ...
  while (no_of_shuffles > 0) {
    for (let i = 0; i < _size; i++) {
      ...
      // instruction
      const index_of_instruction = shuffled.findIndex(
